...
module.exports.run = async (client, message, args, settings, con) => {
  ...
  con.query(`SELECT * FROM userdata WHERE id = '${message.author.id}'`, async (err, rows) => {
    ...
    if(money >= 150) {
      ...
      let card = cards[Math.floor(Math.random() * ((cards.length - 2) - 0 + 1) + 0)];
      let card2 = cards[Math.floor(Math.random() * ((cards.length - 2) - 0 + 1) + 0)];
      let card3 = cards[Math.floor(Math.random() * ((cards.length - 2) - 0 + 1) + 0)];
      ...
      message.channel.send("Here are your cards!");
    }
