...
const fs = require('fs');
//var parseDirectory = require('./modules/FunctionManager.js');
...
var runtime = new Discord.Collection();
...
fs.readdir("./commands/", (err, files) => {
  ...
  jsfiles.forEach((f, i) => {
    ...
    commands.set(props.help.name.split(" ")[0] || props.help.name, props);
  });
});
...
client.on('message', (message) => {
  ...
  let args = message.content.slice(Settings.prefix.length).split(" ").slice(1);
  
  runtime.array().forEach(r => r.run(client, message, args, Settings, con));
  
  if(!message.content.startsWith(Settings.prefix)) return;
  
  var com = commands.get(message.content.slice(Settings.prefix.length).split(" ")[0]);
