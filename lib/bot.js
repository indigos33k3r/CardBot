...
if(mysql) {
  var con = mysql.createConnection({
    host: "localhost",
    user: "root",
    password: Settings.mysql.password.toString(),
    database: Settings.mysql.name.toString()
  });
...
fs.readdir("./commands/", (err, files) => {
  ...
  jsfiles.forEach((f, i) => {
    ...
    if(!props.help.name || !props.help.usage) return new error("ERROR: File " + f + " does not have the proper help defined")
